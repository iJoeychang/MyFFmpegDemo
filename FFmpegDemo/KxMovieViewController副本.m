...
_messageLabel.hidden = YES;
    ...
    _voiceButton.frame = CGRectMake(2, 1, 40, 40);
    ...
    downloadBtn.frame = CGRectMake(2, 1, 40, 40);
    ...
    downloadBtnImageView.image = [UIImage imageNamed:@"player_icon_download@2x.png"];
...
- (BOOL) addFrames: (NSArray *)frames
{
    ...
    NSLog(@"self.playing ========= %hhd",self.playing);
    NSLog(@"_bufferedDuration ========= %f",_bufferedDuration);
    NSLog(@"_maxBufferedDuration ========= %f",_maxBufferedDuration);
...
-(void)downloadFile:(NSString *)path{
    ...
    path = saveFilePath;
     NSString *downloadPath = [self createfileSavePath:path];
...
-(NSString *)createfileSavePath:(NSString *)path{
//    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
//     NSString *cachesDirectory = [paths objectAtIndex:0];
    
    
    NSArray * array = [path componentsSeparatedByString:@"video"];
    ...
    if (str) {
         downloadPath = [folder stringByAppendingString:str] ;//[folder stringByAppendingPathComponent:str];
    }else{
    
         downloadPath = [folder stringByAppendingString:@""] ;
    }
